...
export const imageDocumentHandler = createDocumentHandler<'image'>({
  ...
  onCreateDocument: async ({ title, dataStream }) => {
    ...
    try {
      ...
      console.log(
        `Attempting to generate image using provider: ${provider}, model: ${imageModelId}`,
      );
      ...
      try {
        ...
        // Include the provider information in the data stream and add it to the title
        const modelName = imageModelId.split('-').pop()?.toUpperCase() || '';
        const providerInfo = `${provider.toUpperCase()} ${modelName}`;

        dataStream.writeData({
          type: 'image-delta',
          content: image.base64,
          title: `${title} (generated by ${providerInfo})`,
          metadata: {
        ...
        console.log(
          `Falling back to alternative provider: ${altProvider}, model: ${altImageModelId}`,
        );
        ...
        try {
          ...
          // Include the provider information in the title
          const altModelName =
            altImageModelId.split('-').pop()?.toUpperCase() || '';
          const altProviderInfo = `${altProvider.toUpperCase()} ${altModelName}`;

          dataStream.writeData({
            type: 'image-delta',
            content: image.base64,
            title: `${title} (generated by ${altProviderInfo})`,
            metadata: {
              provider: altProviderInfo,
            },
          });

          console.log(
            `Successfully generated image with fallback provider ${altProvider}`,
          );
          return draftContent;
        } catch (altError) {
          // Both providers failed
          console.error(
            `Error with fallback provider ${altProvider}:`,
            altError,
          );
          throw new Error(
            `Both providers failed to generate images. Please check your API keys and quotas.`,
          );
        }
      }
    } catch (error) {
  ...
  onUpdateDocument: async ({ description, dataStream }) => {
    ...
    try {
      ...
      console.log(
        `Attempting to update image using provider: ${provider}, model: ${imageModelId}`,
      );
      ...
      try {
        ...
        // Include provider information in metadata
        const modelName = imageModelId.split('-').pop()?.toUpperCase() || '';
        const providerInfo = `${provider.toUpperCase()} ${modelName}`;
        ...
        try {
          ...
          console.log(
            `Successfully updated image with fallback provider ${altProvider}`,
          );
          return draftContent;
        } catch (altError) {
          // Both providers failed
          ...
          throw new Error(
            `Both providers failed to update image. Please check your API keys and quotas.`,
          );
        }
      }
    } catch (error) {
